...
The microcontroller can be programmed using the Arduino IDE but we went for another IDE, called PlatformIO, which offers
more advanced features and better project management capabilities. This IDE can be integrated into Visual Studio Code and so 
...
In our application we configure the accelerometer to a range of $\pm 4g$ for reasons that will be explained in Chapter \ref{chap_04:physicalImplem} 
...
\subsection{DRV8833 Motor Driver}

To control the DC motors, we use two DRV8833 motor drivers (shown in Figure \ref{fig:01_drv8833}), which are 
capable of handling motor supply voltages from 6.5V to 45V. They are double full-bridge drivers, allowing 
for bidirectional control of two motors each. However, each channel of each driver can only supply 2A, slightly less than what our motors need in steady state to achieve maximum torque.
For this reason, we connected the two channels of each driver in parallel, to achieve a maximum capability of 4A per motor.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/01_introduction/drv8833.jpeg}
    \caption{DRV8833 motor driver}
\label{fig:01_drv8833}
\end{figure}